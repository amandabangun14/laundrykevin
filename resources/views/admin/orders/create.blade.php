...
@section('title', 'Buat Pesanan - Kevin Laundry')
            ...
            <form method="POST" action="{{ route('admin.orders.store') }}">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="customer_id" name="customer_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('customer_id') border-red-500 @enderror" required>
                                    ...
                                    <div>
                                        <select name="services[0][service_id]" class="service-select w-full px-2 py-1 border rounded" required>
                                            ...
                                            @foreach($services as $service)
                                                <option value="{{ $service->id }}" data-price="{{ $service->price_per_kg }}">{{ $service->name }} - Rp {{ number_format($service->price_per_kg) }}/kg</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                ...
                <div class="flex justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
                    <a href="{{ route('admin.orders') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Batal</a>
